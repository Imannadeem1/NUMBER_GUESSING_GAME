...
try:
    ...
    result.set("Guess a number between 1 to 200 ")
    chances.set(5)
    ...
    def fun():
        chances1.set(chances.get())
        if chances.get()>0:
            if choice.get() > 200 or choice.get()<0:
    ...
    msg = Label(win, text='Guess a number between 1 to 200 ',
    ...
    exit= tk.Button(win, text='Exit', width=40, command=win.destroy,
                 bg="red", activebackground="red", relief=GROOVE)
    exit.place(relx=0.25, rely=1, anchor=S)

    reset = tk.Button(win, text='Restart', width=40, command=restart,
                 bg="green", activebackground="green", relief=GROOVE)
